...
def rbf_kernel(X, Y, tau):
    ...
    n_Y, p_Y = Y.shape
    assert p == p_Y
    K = np.exp(-((X.reshape(n_X, 1, p) - Y.reshape(1, n_Y, p))**2).sum(2) / tau)
    return K
...
'''
def geweke_test(g_mc, g_sc, alpha, l=0.15, use_bonferroni=True):
    ...
    test_statistic = (mean_mc - mean_sc)/np.sqrt(se2_mc + se2_sc)
...
def prob_plot(x, y, plot_type='PP', step = 0.005):
    ...
    x = np.sort(x)
    y = np.sort(y)
    z_min = min(np.min(x), np.min(y))
    z_max = max(np.max(x), np.max(y))

    ecdf = lambda z, x: (x <= z).sum()/float(len(x))
    if plot_type == 'PP':
        pp_x = [ecdf(z, x) for z in np.arange(z_min, z_max, step * (z_max-z_min))]
        pp_y = [ecdf(z, y) for z in np.arange(z_min, z_max, step * (z_max-z_min))]
        ...
        qq_x = np.quantile(x, q)
        qq_y = np.quantile(y, q)
...
def wb_process(n, k=1, l_n=20, rng=None, center=True):
    ...
    W = np.sqrt(1-np.exp(-2/l_n)) * epsilon
    
    for i in range(1, n):
        W[i, :] = W[i-1, :] * np.exp(-1/l_n) + W[i, :]
...
class experiment(object):
    ...
    def run(self):
        ...
        if self._nthreads == 1:
            ...
            pool.close()
            out = [trial for result in out for trial in result]
        return out
...
class sample_experiment(experiment):
    def __init__(self, num_trials, nthreads, seed=None, **kwargs):
        ...
        for attr in ['_sampler', '_num_trials', '_nthreads', '_num_samples', '_burn_in_samples', '_sc_thinning_samples', '_tau', '_alpha']:
    ...
    def run_trial(self):
        ...
        thinned_samples = np.arange(0,int(self._num_samples * self._sc_thinning_samples),self._sc_thinning_samples).astype('int')

        # Geweke test
        samples_p = self._sampler.sample_mc(int(self._num_samples * self._sc_thinning_samples))
        samples_r = self._sampler.sample_sc(int(self._num_samples * self._sc_thinning_samples))
        results_geweke = geweke_test(geweke_functions(samples_p[:,theta_indices]), geweke_functions(samples_r[:, theta_indices]), l=0.04, alpha=self._alpha)

        # MMD test
        samples_q = self._sampler.sample_bc(self._num_samples, burn_in_samples=self._burn_in_samples)
        results_mmd = mmd_test(samples_p[thinned_samples, :][:,theta_indices],samples_q[:, theta_indices], sg.GaussianKernel(10, self._tau), alpha=self._alpha)

        # wild MMD-SC test
        f_kernel = lambda X, Y: rbf_kernel(X, Y, tau=self._tau)
        results_wild = mmd_wb_test(samples_p[thinned_samples, :][:, theta_indices], samples_r[thinned_samples, :][:, theta_indices], f_kernel, alpha=self._alpha)
        
        results = {'geweke': results_geweke, 'mmd': results_backward, 'wild': results_wild}
